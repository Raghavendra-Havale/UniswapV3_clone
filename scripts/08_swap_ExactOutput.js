...
async function exactOutSingle() {
  ...
  poolContract.on(
    ...
    (sender, receipient, amount0, amount1, sqrtPrice, liquidity, tick) => {
      console.log("Swap event detected for ExactOutputSingle:");
      console.log("Sender:", sender);
      console.log("Receipient:", receipient);
      console.log("AmountOutExact:", amount0.toString());
      console.log("AmountInMaximum:", amount1.toString());
  ...
  const inAmount = 0.1;
  const outAmount = 0.001;
  const amountInMaximum = ethers.utils.parseUnits(inAmount.toString(), "18");
  const amountOutExact = ethers.utils.parseUnits(outAmount.toString(), "18");
  ...
  const swapExactOutputTx = await swapRouter
    .connect(signer2)
    .exactOutputSingle(swapExactOutputparams, {
      gasLimit: 1000000,
    });

  const swapExactoutputReceipt = await swapExactOutputTx.wait();
}

async function main() {
  await exactOutSingle();
}

/*
  npx hardhat run --network localhost scripts/08_swap_ExactOutput.js
*/
